...
import {
  StyleSheet, Text, View, Image, TouchableOpacity, Linking, Alert,
} from 'react-native';
...
import {
  OAUTH_CONFIG, OAUTH_APP_NAME, OAUTH_PROVIDER, OAUTH_SCOPES,
} from './constants';
import {
...
async function apiLogin(token) {
  ...
  if (!response.ok) {
    ...
    const error = { text, status: response.status }
    throw error;
  } else {
...
export default class App extends React.Component {
  constructor(props) {
    ...
    this.notificationsSetup = this.notificationsSetup.bind(this);
  }
  ...
  logout() {
    this.manager.deauthorize(OAUTH_PROVIDER)
  ...
  render() {
    const {
      loading, token, username, avatarUrl, loginError, notificationsError,
    } = this.state;
        ...
        <View style={styles.background}>
          ...
          <View style={styles.container}>
            ...
            {notificationsError
              && <>
                ...
                <TouchableOpacity
                  ...
                  onPress={this.notificationsSetup}
                >
            ...
            <Text style={styles.data}>
              {`${DeviceInfo.getApplicationName()} v${DeviceInfo.getVersion()} (${DeviceInfo.getBuildNumber()})`}
            </Text>
          </View>
        </View>
      </Notifications>
